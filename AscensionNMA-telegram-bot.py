...
async def start(update: Update, context: CallbackContext):
    ...
    asyncio.create_task(schedule_message_deletion(msg))
...
async def help_command(query: Update, context: CallbackContext):
    ...
    # Schedule deletion of the help message
    asyncio.create_task(schedule_message_deletion(msg))
...
async def quote_command(query: Update, context: CallbackContext):
    ...
    # Schedule deletion of the quote message
    asyncio.create_task(schedule_message_deletion(msg))
...
async def schedule_message_deletion(*messages):
    ...
    for msg in messages:
        try:
            if msg:
                await msg.delete()
                logger.info(f"Deleted message ID: {msg.message_id}")
        except Exception as e:
